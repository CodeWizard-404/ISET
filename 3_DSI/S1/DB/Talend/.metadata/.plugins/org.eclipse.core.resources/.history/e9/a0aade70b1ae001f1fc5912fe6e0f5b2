 ...
 * @status
 */
public class writers implements TalendJob {
	...
	private final String jobName = "writers";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(writers.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class WritersStruct implements routines.system.IPersistableRow<WritersStruct> {
		final static byte[] commonByteArrayLock_IMDB_writers = new byte[0];
		static byte[] commonByteArray_IMDB_writers = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_IMDB_writers.length) {
					if (length < 1024 && commonByteArray_IMDB_writers.length == 0) {
						commonByteArray_IMDB_writers = new byte[1024];
					} else {
						commonByteArray_IMDB_writers = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_IMDB_writers, 0, length);
				strReturn = new String(commonByteArray_IMDB_writers, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_IMDB_writers, 0, length);
				strReturn = new String(commonByteArray_IMDB_writers, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_IMDB_writers) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("writer");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("writer");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:2R9/tNo3kB+IN6gu34OEPmW0tYnkwORa3l+NZGUYhZBLKTfBMPt/zg==");
...
// ###############################
				...
				int limit_tFileInputDelimited_1 = 550000;
				try {
...
// Start of branch "row1"
						if (row1 != null) {
							...
							row1.primaryProfession.contains("writer") && !row1.birthYear.equals("\\N")
...
// # Output table : 'Writers'
									Writers_tmp.nconst = row1.nconst;
									Writers_tmp.primaryName = row1.primaryName;
									Writers = Writers_tmp;
// ###############################
...
// Start of branch "Writers"
								if (Writers != null) {
									...
									if (Writers.nconst == null) {
										pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(1, Writers.nconst);
									}

									if (Writers.primaryName == null) {
										pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(2, Writers.primaryName);
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final writers writersClass = new writers();

		int exitCode = writersClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = writers.class.getClassLoader()
					.getResourceAsStream("imdb/writers_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = writers.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : writers");
		}
...
/************************************************************************************************
 * 60306 characters generated by Talend Open Studio for Data Integration on the
 * 28 novembre 2024, 1:30:13 AM CET
 ************************************************************************************************/