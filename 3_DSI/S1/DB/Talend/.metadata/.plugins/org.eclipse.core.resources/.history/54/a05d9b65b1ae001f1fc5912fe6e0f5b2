 ...
 * @status
 */
public class actors implements TalendJob {
	...
	private final String jobName = "actors";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(actors.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class actorsStruct implements routines.system.IPersistableRow<actorsStruct> {
		final static byte[] commonByteArrayLock_IMDB_actors = new byte[0];
		static byte[] commonByteArray_IMDB_actors = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_IMDB_actors.length) {
					if (length < 1024 && commonByteArray_IMDB_actors.length == 0) {
						commonByteArray_IMDB_actors = new byte[1024];
					} else {
						commonByteArray_IMDB_actors = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_IMDB_actors, 0, length);
				strReturn = new String(commonByteArray_IMDB_actors, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_IMDB_actors, 0, length);
				strReturn = new String(commonByteArray_IMDB_actors, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_IMDB_actors) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (dbschema_tDBOutput_1 == null || dbschema_tDBOutput_1.trim().length() == 0) {
					tableName_tDBOutput_1 = ("actor");
				} else {
					tableName_tDBOutput_1 = dbschema_tDBOutput_1 + "\".\"" + ("actor");
				}
				...
				final String decryptedPassword_tDBOutput_1 = routines.system.PasswordEncryptUtil.decryptPassword(
						"enc:routine.encryption.key.v1:oNm/+PAUW1xzV73JgsAvRU3HIXR99JHKevQE0VnAZRoeebmz2NwViw==");
...
// Start of branch "row1"
						if (row1 != null) {
							...
							(row1.primaryProfession.contains("actor") || row1.primaryProfession.contains("actress"))
									&& !row1.birthYear.equals("\\N") && !row1.deathYear.equals("\\N")
...
// Start of branch "actors"
								if (actors != null) {
									...
									if (actors.nconst == null) {
										pstmt_tDBOutput_1.setNull(1, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(1, actors.nconst);
									}

									if (actors.primaryName == null) {
										pstmt_tDBOutput_1.setNull(2, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(2, actors.primaryName);
									}

									if (actors.genre == null) {
										pstmt_tDBOutput_1.setNull(3, java.sql.Types.VARCHAR);
									} else {
										pstmt_tDBOutput_1.setString(3, actors.genre);
									}
...
// ###############################      
	...
	public static void main(String[] args) {
		final actors actorsClass = new actors();

		int exitCode = actorsClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = actors.class.getClassLoader()
					.getResourceAsStream("imdb/actors_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = actors.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : actors");
		}
...
/************************************************************************************************
 * 61405 characters generated by Talend Open Studio for Data Integration on the
 * 28 novembre 2024, 10:44:50 AM CET
 ************************************************************************************************/